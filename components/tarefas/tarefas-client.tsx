...
import {
  ...
  Paperclip, Upload, FileText, Image, Download, Filter,
} from "lucide-react"
...
export function TarefasClient({ empresaId, blocosInit, tarefasInit }: Props) {
  ...
  function getStatusFromOverId(overId: string): StatusTarefa | null {
    // Se o over é uma coluna diretamente
    const col = STATUS_COLS.find((s) => s.id === overId)
    if (col) return col.id
    // Se o over é uma tarefa, pegar o status dessa tarefa
    const tarefa = tarefas.find((t) => t.id === overId)
  ...
  async function handleDragEnd(e: DragEndEvent) {
    ...
    const statusDestino = getStatusFromOverId(over.id as string)
    if (statusDestino) {
      // Sempre atualizar no banco (o handleDragOver já atualizou o estado local)
      const tarefaOriginal = tarefas.find((t) => t.id === tarefaId)
      // Garantir que o estado local reflete o destino
      setTarefas((p) => p.map((t) => t.id === tarefaId ? { ...t, status: statusDestino } : t))
      // Salvar no banco
      const { error } = await supabase.from("tarefas").update({ status: statusDestino }).eq("id", tarefaId)
      if (error) {
        toast.error("Erro ao salvar. Recarregue a página.")
      } else if (tarefaOriginal && tarefaOriginal.status !== statusDestino) {
...
function SortableCard({ tarefa, blocos, onClick, onEdit }: {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <KanbanCard tarefa={tarefa} blocos={blocos} onClick={onClick} onEdit={onEdit} />
    </div>
  )
}

// ── Kanban Card ──────────────────────────────────────────
function KanbanCard({ tarefa, blocos, isDragging, onClick, onEdit }: {
  ...
  return (
    <div className={cn(
      "group rounded-xl p-3.5 cursor-grab active:cursor-grabbing transition-all duration-200",
