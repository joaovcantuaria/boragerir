...
export function AdminDashboard({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className={`text-2xl font-black ${t.text}`}>Dashboard</h1>
            ...
            <div key={c.label} className={`${t.cardBg} border ${t.border} rounded-2xl p-4`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-xl font-black ${t.text}`}>{c.valor}</p>
                </div>
              </div>
            </div>
          )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`${t.cardBg} border ${t.border} rounded-2xl p-5`}>
          <h3 className={`text-sm font-bold ${t.text} mb-4`}>Distribuição por plano</h3>
          {dadosPizza.length > 0 ? (
              ...
              <div className="space-y-2">
                {dadosPizza.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={t.textMuted6}>{d.name}</span>
                    <span className={`${t.text} font-bold ml-auto`}>{d.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ) : (
            <p className={`${t.textMuted2} text-sm py-8 text-center`}>Nenhum dado ainda</p>
          )}
        </div>

        <div className={`${t.cardBg} border ${t.border} rounded-2xl p-5`}>
          <h3 className={`text-sm font-bold ${t.text} mb-4`}>Receita por tipo</h3>
            ...
            {[
              ...
              <div key={item.label}>
                <div className="flex justify-between text-sm mb-1">
                  <span className={t.textMuted3}>{item.label}</span>
                  <span className={`${t.text} font-bold`}>{formatarMoeda(item.valor)}</span>
                </div>
                <div className={`h-2 ${t.subBg} rounded-full`}>
      ...
      {/* Empresas recentes + Assinaturas */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`${t.cardBg} border ${t.border} rounded-2xl p-5`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-bold ${t.text}`}>Empresas recentes</h3>
          ...
          <div className="space-y-2">
            {empresasRecentes.length > 0 ? empresasRecentes.map((e) => (
              <div key={e.id} className={`flex items-center gap-3 py-2 border-b ${t.borderLight} last:border-0 cursor-pointer ${t.hoverBg} rounded-lg px-2 transition-colors`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-xs ${t.textMuted}`}>{e.area_atuacao}</p>
                </div>
              ...
              <p className={`${t.textMuted2} text-sm py-4 text-center`}>Nenhuma empresa ainda</p>
            )}
          </div>
        </div>
        ...
        <div className={`${t.cardBg} border ${t.border} rounded-2xl p-5`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-sm font-bold ${t.text}`}>Assinaturas recentes</h3>
          ...
          <div className="space-y-2">
            {assinaturas.length > 0 ? assinaturas.map((a) => {
              ...
              return (
                <div key={a.id} className={`flex items-center gap-3 py-2 border-b ${t.borderLight} last:border-0`}>
                  <Icon className={`w-4 h-4 ${badge.cor} shrink-0`} />
                  <div className="flex-1">
                    <p className={`text-sm font-semibold ${t.text} capitalize`}>{a.plano} — {a.periodicidade}</p>
              ...
              <p className={`${t.textMuted2} text-sm py-4 text-center`}>Nenhuma assinatura ainda</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className={`text-sm font-bold ${t.text}`}>Tickets abertos ({ticketsAbertos.length})</h3>
            </div>
          ...
          <div className="space-y-2">
            {ticketsAbertos.map((tk) => (
              <div key={tk.id} className={`flex items-center justify-between py-2 border-b ${t.borderLight} last:border-0 cursor-pointer`}
                onClick={() => router.push("/admin/suporte")}>
                <div>
                  <p className={`text-sm font-semibold ${t.text}`}>{tk.assunto}</p>
                  <p className={`text-xs ${t.textMuted}`}>{format(parseISO(tk.created_at), "dd/MM HH:mm")}</p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full border ${
                  tk.prioridade === "urgente" ? "bg-red-500/10 text-red-400 border-red-500/20" :
                  tk.prioridade === "alta" ? "bg-orange-500/10 text-orange-400 border-orange-500/20" :
