  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 dark:text-white/40 text-sm">{atendimentos.length} atendimento(s) registrado(s)</p>
      </div>

      {/* Cards resumo */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total", valor: atendimentos.length, cor: "text-gray-900 dark:text-white", bg: "bg-gray-100 dark:bg-white/5" },
          ...
          <div key={c.label} className={`${c.bg} border border-gray-100 dark:border-white/10 rounded-2xl p-4`}>
      ...
      {filtrados.length > 0 ? (
        <div className="space-y-2">
          {filtrados.map((a) => {
            ...
            return (
              <div key={a.id} className="bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-white/10 rounded-2xl overflow-hidden">
                {/* Header do atendimento */}
                <button
                  className="w-full text-left px-5 py-4 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors"
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {carregando === a.id ? (
                        ...
                        <ChevronUp className="w-4 h-4 text-gray-300 dark:text-white/30" />
                      ) : (
                        <ChevronDown className="w-4 h-4 text-gray-300 dark:text-white/30" />
                      )}
                    </div>
                  </div>
                </button>
                ...
                {isExpanded && msgs.length > 0 && (
                  <div className="border-t border-gray-100 dark:border-white/10 px-5 py-4 space-y-3 bg-gray-50/50 dark:bg-white/[0.01]">
                    {msgs.map((msg, i) => (
                      <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-xs ${
                          msg.role === "assistant" ? "bg-primary" : "bg-gray-100 dark:bg-white/10"
                        }`}>
                          {msg.role === "assistant" ? "🌟" : "👤"}
                        </div>
                ...
                {isExpanded && msgs.length === 0 && !carregando && (
                  <div className="border-t border-gray-100 dark:border-white/10 px-5 py-3 text-xs text-gray-300 dark:text-white/30 text-center">
