...
export function AdminSuporteClient({ tickets: init }: { tickets: Ticket[] }) {
  ...
  async function responder(ticket: Ticket) {
    ...
    if (res.ok) {
      setTickets((prev) => prev.map((tk) =>
        tk.id === ticket.id ? { ...tk, resposta_admin: resposta, status: "resolvido", respondido_em: new Date().toISOString() } : tk
      ))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className={`text-2xl font-black ${t.text}`}>Suporte</h1>
        <p className={`${t.textMuted} text-sm`}>{tickets.filter((tk) => tk.status === "aberto").length} tickets abertos</p>
      </div>
      ...
      {filtrados.length > 0 ? (
        <div className="space-y-3">
          {filtrados.map((ticket) => (
            <div key={ticket.id} className={`${t.cardBg} border ${t.border} rounded-2xl overflow-hidden`}>
              {/* Header do ticket */}
              <div
                className={`flex items-center justify-between px-5 py-4 cursor-pointer ${t.rowHover} transition-colors`}
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className={`text-sm font-semibold ${t.text}`}>{ticket.assunto}</p>
                    <p className={`text-xs ${t.textMuted}`}>{ticket.empresas?.nome} · {format(parseISO(ticket.created_at), "dd/MM HH:mm")}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {aberto === ticket.id ? <ChevronUp className={`w-4 h-4 ${t.textMuted}`} /> : <ChevronDown className={`w-4 h-4 ${t.textMuted}`} />}
                </div>
              </div>

              {/* Detalhes */}
              {aberto === ticket.id && (
                <div className={`px-5 pb-5 border-t ${t.border} space-y-4`}>
                  {/* Mensagem */}
                  <div className={`${t.subBg} rounded-xl p-4 mt-4`}>
                    <p className={`text-xs ${t.textMuted} mb-1`}>Mensagem do cliente</p>
                    <p className={`text-sm ${t.text}`}>{ticket.mensagem}</p>
                  </div>
                  ...
                  {ticket.resposta_admin && (
                    <div className="bg-primary/10 border border-primary/20 rounded-xl p-4">
                      ...
                      <p className={`text-sm ${t.text}`}>{ticket.resposta_admin}</p>
                    </div>
                  )}
