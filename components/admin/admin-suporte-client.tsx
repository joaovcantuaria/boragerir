  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white/40 text-sm">{tickets.filter((t) => t.status === "aberto").length} tickets abertos</p>
      </div>
      ...
      {filtrados.length > 0 ? (
        <div className="space-y-3">
          {filtrados.map((ticket) => (
            <div key={ticket.id} className="bg-[#1a1a1a] border border-white/10 rounded-2xl overflow-hidden">
              {/* Header do ticket */}
              <div
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-white">{ticket.assunto}</p>
                    <p className="text-xs text-white/40">{ticket.empresas?.nome} · {format(parseISO(ticket.created_at), "dd/MM HH:mm")}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {aberto === ticket.id ? <ChevronUp className="w-4 h-4 text-white/40" /> : <ChevronDown className="w-4 h-4 text-white/40" />}
                </div>
              </div>
              ...
              {aberto === ticket.id && (
                <div className="px-5 pb-5 border-t border-white/10 space-y-4">
                  ...
                  <div className="bg-white/5 rounded-xl p-4 mt-4">
                    <p className="text-xs text-white/40 mb-1">Mensagem do cliente</p>
                    <p className="text-sm text-white">{ticket.mensagem}</p>
                  </div>
                  ...
                  {ticket.resposta_admin && (
                    <div className="bg-primary/10 border border-primary/20 rounded-xl p-4">
                      ...
                      <p className="text-sm text-white">{ticket.resposta_admin}</p>
                    </div>
                  )}
