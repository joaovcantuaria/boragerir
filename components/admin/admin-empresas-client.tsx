...
export function AdminEmpresasClient({ empresas: init }: { empresas: Empresa[] }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Empresas</h1>
        <p className="text-gray-400 dark:text-white/40 text-sm">{empresas.length} empresas cadastradas</p>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-60">
          ...
          <input
            className="w-full bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/30 focus:outline-none focus:border-[#F26E1D]"
          ...
          {["todos", "gratuito", "basico", "profissional"].map((p) => (
            <button key={p} onClick={() => setFiltroPlano(p)}
              className={`px-3 py-2 rounded-xl text-xs font-bold transition-all capitalize ${
                filtroPlano === p ? "bg-primary text-white" : "bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-white/50 hover:bg-gray-200 dark:hover:bg-white/10"
              }`}>
              {p}
            </button>
          ))}
      ...
      <div className="bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-white/10 rounded-2xl overflow-hidden">
        <div className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-4 px-5 py-3 border-b border-gray-100 dark:border-white/10 text-xs font-bold text-gray-300 dark:text-white/30 uppercase tracking-wider">
        ...
        {filtradas.length > 0 ? filtradas.map((e) => {
          ...
          return (
            <div key={e.id} className="grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-4 px-5 py-4 border-b border-gray-50 dark:border-white/5 last:border-0 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors items-center">
              {/* Empresa */}
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-300 dark:text-white/30">{docFormatado}</p>
                  <p className="text-xs text-gray-300 dark:text-white/20">{format(parseISO(e.created_at), "dd/MM/yyyy")}</p>
                </div>
              </div>

              {/* Contato */}
              <div className="min-w-0">
                <p className="text-xs text-gray-600 dark:text-white/60 truncate">{e.email}</p>
                <p className="text-xs text-gray-300 dark:text-white/30">{formatarTelefone(e.telefone)}</p>
              </div>

              {/* Área */}
              <p className="text-xs text-gray-500 dark:text-white/50 truncate">{e.area_atuacao}</p>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 dark:text-white/40 hover:text-gray-900 dark:hover:text-white transition-colors"
                  title="Ver detalhes"
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-yellow-400/60 hover:text-yellow-400 transition-colors"
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 transition-colors ${e.plano_ativo ? "text-emerald-400 hover:text-red-400" : "text-red-400 hover:text-emerald-400"}`}
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10 text-red-400/60 hover:text-red-400 transition-colors"
          ...
          <div className="py-12 text-center text-gray-300 dark:text-white/30">
